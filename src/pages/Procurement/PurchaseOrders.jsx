...
export default function PurchaseOrders() {
  ...
  return (
    <div>
              ...
              : filtered.map(po => {
                ...
                return (
                  <tr key={po.id} onClick={() => setViewPO(po)} style={{ cursor: 'pointer' }}
                    ...
                    <td className="td-mono" style={{ color: 'var(--teal)' }}>${parseFloat(po.total_amount || 0).toFixed(2)}</td>
      ...
      {/* View PO modal */}
      <ModalDialog open={!!viewPO} onClose={() => setViewPO(null)} title={viewPO ? `${viewPO.po_number} — ${viewPO.supplier_name}` : ''} size="lg">
        {viewPO && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16, fontSize: 13 }}>
              ...
              <div><span style={{ color: 'var(--text-dim)' }}>Status:</span> <StatusBadge status={viewPO.status} /></div>
      ...
      {/* Create PO Modal */}
      <ModalDialog open={modalOpen} onClose={() => setModalOpen(false)} title="Create Purchase Order" size="xl">
