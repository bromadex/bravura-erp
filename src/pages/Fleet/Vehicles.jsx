...
export default function Vehicles() {
  ...
  const [form, setForm] = useState({
    reg: '', type: '', description: '', driver_id: '', driver_name: '', status: 'Active'
  })

  // Fetch fuel consumption for each vehicle
  useEffect(() => {
  ...
  const openModal = (vehicle = null) => {
    if (vehicle) {
      ...
      setForm({
        ...
        driver_id: vehicle.driver_id || '', driver_name: vehicle.driver_name || '',
        status: vehicle.status || 'Active'
      })
    } else {
      setEditing(null)
      setForm({ reg: '', type: '', description: '', driver_id: '', driver_name: '', status: 'Active' })
    }
    setModalOpen(true)
  }
        ...
        {loading ? (
          ...
          vehicles.map(v => (
            <div key={v.id} className="card" style={{ padding: 16 }}>
              ...
              <div style={{ marginTop: 8, fontSize: 12 }}>⛽ Total Fuel: <strong>{fuelMap[v.reg]?.toLocaleString() || 0} L</strong></div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-row">
                <div className="form-group"><label>Registration *</label><input className="form-control" required value={form.reg} onChange={e => setForm({...form, reg: e.target.value})} /></div>
