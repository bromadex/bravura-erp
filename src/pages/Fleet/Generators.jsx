        ...
        {loading ? <div>Loading...</div> : generators.length === 0 ? <div className="empty-state">No generators added</div> : generators.map(g => (
          <div key={g.id} className="card" style={{ padding: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 8 }}>
              {canEdit && <button className="btn btn-secondary btn-sm" onClick={() => openRunModal(g.id)}><span className="material-icons">schedule</span> Log Run</button>}
              {canEdit && <button className="btn btn-secondary btn-sm" onClick={() => openModal(g)}><span className="material-icons">edit</span></button>}
              {canDelete && <button className="btn btn-danger btn-sm" onClick={() => handleDelete(g)}><span className="material-icons">delete</span></button>}
            </div>
          </div>
        ))}
      </div>
              ...
              {genRunLogs.map(log => {
                ...
                return (
                  <tr key={log.id}>
                    ...
                    <td>{log.fuel_used}</td>
                    <td>{log.notes || '-'}</td>
                    <td>{canDelete && <button className="btn btn-danger btn-sm" onClick={() => handleDeleteRunLog(log.id)}><span className="material-icons">delete</span></button>}</td>
                  </tr>
                )
              })}
        ...
        <div className="overlay" onClick={() => setModalOpen(false)}>
          <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-row"><div className="form-group"><label>Generator Code *</label><input className="form-control" required value={form.gen_code} onChange={e => setForm({...form, gen_code: e.target.value})} /></div><div className="form-group"><label>Name</label><input className="form-control" value={form.gen_name} onChange={e => setForm({...form, gen_name: e.target.value})} /></div></div>
              <div className="form-row"><div className="form-group"><label>Location</label><input className="form-control" value={form.location} onChange={e => setForm({...form, location: e.target.value})} /></div><div className="form-group"><label>Capacity (kVA)</label><input type="number" className="form-control" value={form.capacity} onChange={e => setForm({...form, capacity: e.target.value})} /></div></div>
              <div className="form-row"><div className="form-group"><label>Status</label><select className="form-control" value={form.status} onChange={e => setForm({...form, status: e.target.value})}><option>Running</option><option>Stopped</option><option>Maintenance</option><option>Offline</option></select></div><div className="form-group"><label>Last Service Date</label><input type="date" className="form-control" value={form.service_date} onChange={e => setForm({...form, service_date: e.target.value})} /></div></div>
        ...
        <div className="overlay" onClick={() => setRunModalOpen(false)}>
          <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleRunSubmit}>
              <div className="form-row"><div className="form-group"><label>Generator</label><select className="form-control" required value={runForm.gen_id} onChange={e => setRunForm({...runForm, gen_id: e.target.value})}><option value="">Select</option>{generators.map(g => <option key={g.id} value={g.id}>{g.gen_code} - {g.gen_name}</option>)}</select></div><div className="form-group"><label>Date</label><input type="date" className="form-control" required value={runForm.date} onChange={e => setRunForm({...runForm, date: e.target.value})} /></div></div>
              <div className="form-row"><div className="form-group"><label>Start Time</label><input type="time" className="form-control" value={runForm.start_time} onChange={e => setRunForm({...runForm, start_time: e.target.value})} /></div><div className="form-group"><label>End Time</label><input type="time" className="form-control" value={runForm.end_time} onChange={e => setRunForm({...runForm, end_time: e.target.value})} /></div></div>
              <div className="form-row"><div className="form-group"><label>Engine Hours</label><input type="number" step="0.1" className="form-control" placeholder="Total hours run" value={runForm.hours} onChange={e => setRunForm({...runForm, hours: e.target.value})} /></div><div className="form-group"><label>Fuel Used (L)</label><input type="number" step="0.1" className="form-control" value={runForm.fuel_used} onChange={e => setRunForm({...runForm, fuel_used: e.target.value})} /></div></div>
              ...
              <div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={() => setRunModalOpen(false)}>Cancel</button><button type="submit" className="btn btn-primary">Save Log</button></div>
            </form>
          </div>
        </div>
      )}
