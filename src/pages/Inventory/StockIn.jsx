  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowModal(true)}><span className="material-icons">add<\/span> Stock In<\/button>
      <\/div>
      <div className="table-wrap">
        <table>
          <thead><tr><th>#<\/th><th>Date<\/th><th>Item<\/th><th>Category<\/th><th>Qty<\/th><th>Delivered By<\/th><th>Received By<\/th><th>Notes<\/th><\/tr><\/thead>
          <tbody>
            {loading ? <tr><td colSpan="8" style={{ textAlign:'center', padding:40 }}>Loading...<\/td><\/tr> : stockInTransactions.length === 0 ? <tr><td colSpan="8" style={{ textAlign:'center', padding:40 }}>No stock in records<\/td><\/tr> : stockInTransactions.map((tx, idx) => (
              <tr key={tx.id}>
                <td>{idx+1}<\/td>
...
function StockInModal({ items, onClose, onSave }) {
  const [form, setForm] = useState({ itemId: '', quantity: 1, date: new Date().toISOString().split('T')[0], deliveredBy: '', receivedBy: '', notes: '' })
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-title"><span className="material-icons">inventory_2<\/span> Stock <span>In<\/span><\/div>
        <form onSubmit={handleSubmit}>
          ...
          {selectedItem && <div style={{ background:'var(--surface2)', padding:8, borderRadius:8, marginBottom:12, fontSize:12, display:'flex', gap:16 }}><span>Unit: <strong>{selectedItem.unit || 'pcs'}<\/strong><\/span><span>Current Stock: <strong>{selectedItem.balance}<\/strong><\/span><\/div>}
          <div className="form-row">
            <div className="form-group"><label>Quantity *<\/label><input type="number" className="form-control" required min="1" value={form.quantity} onChange={e => setForm({...form, quantity: parseInt(e.target.value) || 0})} /><\/div>
          ...
          <div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={onClose}>Cancel<\/button><button type="submit" className="btn btn-primary" disabled={loading}>{loading ? 'Processing...' : 'Confirm Stock In'}<\/button><\/div>
        <\/form>
      <\/div>
    <\/div>
  )
