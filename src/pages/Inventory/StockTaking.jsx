    ...
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowModal(true)}>
          <span className="material-icons" style={{ fontSize: 18 }}>fact_check</span> New Stock Take
        </button>
      </div>
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Date</th><th>Item</th><th>System Qty</th><th>Counted</th><th>Variance</th><th>Done By</th><th>Notes</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="7" style={{ textAlign: 'center', padding: 40 }}>No stock take records</td></tr>
            ) : (
              stockTakes.map((st) => (
                <tr key={st.id}>
                  ...
                  <td style={{ color: st.variance >= 0 ? 'var(--green)' : 'var(--red)' }}>
    ...
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-title">
          <span className="material-icons" style={{ fontSize: 20, marginRight: 8 }}>fact_check</span>
          Stock <span>Take</span>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <select className="form-control" required value={form.itemId} onChange={e => setForm({...form, itemId: e.target.value})}>
              ...
              {items.map(i => <option key={i.id} value={i.id}>{i.name} ({i.unit || 'pcs'}) — Current: {i.balance}</option>)}
            </select>
          </div>
          {selectedItem && (
            <div style={{ background: 'var(--surface2)', padding: 8, borderRadius: 8, marginBottom: 12, fontSize: 12 }}>
              <span><span className="material-icons" style={{ fontSize: 12, verticalAlign: 'middle' }}>inventory</span> Current Balance: <strong>{selectedItem.balance} {selectedItem.unit || 'pcs'}</strong></span>
              {variance !== 0 && (
                <span style={{ marginLeft: 16, color: variance > 0 ? 'var(--green)' : 'var(--red)' }}>
                  Adjustment: {variance > 0 ? '+' : ''}{variance}
                </span>
              )}
            </div>
          )}
          <div className="form-row">
            <div className="form-group">
              <label><span className="material-icons" style={{ fontSize: 14, marginRight: 4 }}>straighten</span> Counted Quantity *</label>
              <input type="number" className="form-control" required min="0" value={form.countedQty} onChange={e => setForm({...form, countedQty: parseInt(e.target.value) || 0})} />
            </div>
            <div className="form-group">
          ...
          <div className="form-row">
            <div className="form-group">
              <label><span className="material-icons" style={{ fontSize: 14, marginRight: 4 }}>badge</span> Counted By</label>
              <input className="form-control" value={form.countedBy} onChange={e => setForm({...form, countedBy: e.target.value})} />
            </div>
            <div className="form-group">
          ...
          <div className="modal-actions">
            ...
            <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? 'Processing...' : 'Apply Count'}</button>
          </div>
        </form>
      </div>
    </div>
