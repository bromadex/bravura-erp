    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20, alignItems: 'flex-end' }}>
        ...
        {(dateFrom || dateTo || search !== '' || filterType !== 'ALL') && (
          <button className="btn btn-secondary btn-sm" onClick={() => { setDateFrom(''); setDateTo(''); setSearch(''); setFilterType('ALL') }}>
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Date</th><th>Type</th><th>Item</th><th>Category</th><th>Qty</th><th>Reference</th><th>User</th><th>Notes</th><th></th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
                ...
                <tr key={tx.id}>
                  ...
                  <td>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: getTypeColor(tx.type) }}>
                      <span className="material-icons" style={{ fontSize: 16 }}>{getTypeIcon(tx.type)}</span>
                      {tx.type}
                    </span>
                  </td>
