...
import { useLeave } from '../../contexts/LeaveContext'   // ✅ Tier 2
...
export default function Employees() {
  ...
  const { isOnLeave } = useLeave()   // ✅ Tier 2 — reads from cache, no DB calls
  ...
  // Fetch leave types for initial balances UI
  useEffect(() => {
  ...
  // ── ✅ Tier 2: status badge helper using real CSS classes ────────
  // Checks the LeaveContext cache first (zero DB calls).
  // Employee's DB .status may still say "Active" even if on approved
  // leave today — isOnLeave() catches that case.
  const statusColors = {
    Active:      'badge-green',
  ...
  const openCertModal = (cert = null) => {
    if (cert) {
      setEditingCert(cert)
      setCertForm({ id: cert.id, certification_name: cert.certification_name, issuing_body: cert.issuing_body || '', issue_date: cert.issue_date || '', expiry_date: cert.expiry_date || '', document_url: cert.document_url || '', notes: cert.notes || '' })
    } else {
      setEditingCert(null)
      ...
      <div>
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input type="text" className="form-control" placeholder="New skill"
              value={newSkill.name} onChange={e => setNewSkill({ ...newSkill, name: e.target.value })}
              style={{ flex: 2 }} />
            <select className="form-control" value={newSkill.proficiency}
                  ...
                  return (
                    <tr key={cert.id}>
                      ...
                      <td>
                        ...
                        : expiring ? <span className="badge badge-yellow">Expiring Soon</span>
                                   : <span className="badge badge-green">Valid</span>}
                      </td>
                      <td style={{ display: 'flex', gap: 4 }}>
                        <button className="btn btn-secondary btn-sm" onClick={() => openCertModal(cert)}>
                          <span className="material-icons">edit</span>
                        </button>
                        <button className="btn btn-danger btn-sm" onClick={() => handleDeleteCertification(cert.id, cert.certification_name)}>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 20 }}>
        <div className="form-row">
          <div className="form-group">
            ...
            <input type="text" className="form-control" placeholder="Name, phone, ID, or designation..."
          ...
          return (
            <div key={emp.id} className="card" style={{ padding: 16, cursor: 'pointer' }} onClick={() => openViewModal(emp)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div>
                  {/* ✅ Tier 2: badge uses real CSS class, shows ON LEAVE from cache */}
                  <span className={`badge ${statusCls}`}>{statusLabel}</span>
              ...
              <div style={{ marginTop: 8, borderTop: '1px solid var(--border)', paddingTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 12 }}>
                  ...
                  {' '}This week: <strong>{weeklyStats.totalHours.toFixed(1)}h</strong> (OT: {weeklyStats.totalOvertime.toFixed(1)}h)
                </span>
                {canEdit && (
          ...
          <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
            ...
            {accountInfo && (
              <div className="info-box" style={{ marginBottom: 16, background: 'rgba(52,211,153,.1)', borderColor: 'rgba(52,211,153,.3)' }}>
                ...
                <button className="btn btn-secondary btn-sm"
                  onClick={() => navigator.clipboard.writeText(`Username: ${accountInfo.username}\nPassword: ${accountInfo.password}`)}>
                  Copy Credentials
                </button>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                  ...
                  {manualEmployeeId ? (
                    <div style={{ marginTop: 8 }}>
                      <input className="form-control" value={form.employee_number}
              ...
              {/* Initial leave balances — new employees only */}
              {!editing && leaveTypesList.length > 0 && (
                <div className="card" style={{ marginTop: 16, padding: 16, background: 'var(--surface2)' }}>
                  ...
                  <p style={{ fontSize: 11, marginBottom: 12, color: 'var(--text-dim)' }}>
                    Enter starting days for this employee. Leave at zero if not applicable.
                  </p>
              ...
              {/* System account — new employees only */}
              {!editing && (
