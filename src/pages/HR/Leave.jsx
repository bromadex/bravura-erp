...
//
// TIER 1 FIXES:
// 1. Status badges now match the rest of the app: a single <span class="badge badge-X">
//    instead of <div class="badge"><span class="bg-X">. The CSS only defines
//    .badge-green/.badge-red/.badge-yellow/.badge-blue/.badge-gold/.badge-purple
//    so the old "bg-green" inner class never had any styling at all.
//
// 2. ApprovalCard buttons now use .btn-primary / .btn-danger which actually
//    exist in index.css. The old .btn-green and .btn-red classes were never
//    defined and rendered as plain unstyled buttons.
//
// 3. Edit button only appears for status === 'draft'. Per the spec, once a
//    request is submitted (pending_supervisor or pending_hr) it is locked from
//    edit — only Cancel is allowed. The previous code let users edit pending
//    requests, which would have let them silently re-submit and bypass the
//    approval trail.
//
// Removed unused canViewAll variable.
...
export default function Leave() {
  ...
  useEffect(() => {
    if (approverEmployeeId) fetchPendingSupervisor()
  ...
  const editDraft = (request) => {
    // ✅ TIER 1 FIX: Only drafts may be edited. Calling code already guards this,
    // but we double-check here in case someone forces it via dev tools.
    if (request.status !== 'draft') {
      toast.error('Only drafts can be edited. Cancel and resubmit instead.')
      return
    }
    setForm({
  ...
  // ✅ TIER 1 FIX: Status → real CSS badge class mapping. The CSS in index.css
  // defines .badge-green / .badge-red / .badge-yellow / .badge-blue / .badge-gold
  // / .badge-purple. There is no .badge-gray or "bg-X" class in the project.
  const statusBadgeClass = (status) => {
    switch (status) {
      case 'draft':              return 'badge badge-yellow'
      case 'pending_supervisor': return 'badge badge-blue'
      case 'pending_hr':         return 'badge badge-blue'
      case 'approved':           return 'badge badge-green'
      case 'rejected':           return 'badge badge-red'
      case 'cancelled':          return 'badge badge-purple'
    ...
    return (
      <div style={{ padding: 12, background: 'var(--surface2)', borderRadius: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
          {/* ✅ TIER 1 FIX: btn-green/btn-red don't exist; use real classes */}
      ...
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
        ...
        <div className="card" style={{ flex: 2, minWidth: 300, padding: 20 }}>
          ...
          {form.leave_type_id && (
            <div style={{ marginBottom: 16, background: 'var(--surface2)', padding: 12, borderRadius: 8 }}>
              <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>
                Your balance for {selectedLeaveType?.name}
              </div>
          ...
          <div className="form-group">
            ...
            <div className="form-control" style={{ background: 'var(--surface2)', cursor: 'not-allowed' }}>
          ...
          <div className="modal-actions" style={{ justifyContent: 'flex-start', marginTop: 16 }}>
            <button type="button" className="btn btn-secondary" onClick={handleSaveDraft} disabled={submitting}>
              <span className="material-icons" style={{ fontSize: 16 }}>save</span> Save Draft
            </button>
                  ...
                  <div key={req.id} style={{ padding: 12, background: 'var(--surface2)', borderRadius: 8 }}>
                    <div style={{
                      ...
                      <div>
                        ...
                        {/* ✅ TIER 1 FIX: single span with proper class — was wrapped div before */}
                        <span className={statusBadgeClass(req.status)} style={{ marginTop: 6 }}>
                          {req.status.replace(/_/g, ' ')}
                        </span>
                      </div>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {/* ✅ TIER 1 FIX: edit only on draft */}
                        {req.status === 'draft' && (
                    ...
                    {req.attachment_url && (
                      <div style={{ marginTop: 4 }}>
                        <a href={req.attachment_url} target="_blank" rel="noopener noreferrer"
                          ...
                          <span className="material-icons" style={{ fontSize: 12 }}>attachment</span>
                          Attachment
                        </a>
                      </div>
                    )}
      ...
      {commentModal.open && (
        <div className="overlay" onClick={() => setCommentModal({ open: false, requestId: null, comment: '' })}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button className="btn btn-secondary"
                onClick={() => setCommentModal({ open: false, requestId: null, comment: '' })}>
                Cancel
              </button>
