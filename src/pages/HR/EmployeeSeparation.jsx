...
const SEP_STATUSES      = ['Pending', 'In Progress', 'Completed', 'Cancelled']
...
const emptyNewForm = () => ({
  employee_id: '', template_id: '', resignation_date: '', last_working_day: '', reason: '',
})
...
export default function EmployeeSeparation() {
  const { user }  = useAuth()
  const canEdit   = useCanEdit('hr', 'employee-separation')

  const [mainTab, setMainTab] = useState('separations')
  ...
  const [detailExitNotes,  setDetailExitNotes]  = useState('')
  const [detailSettlement, setDetailSettlement] = useState('')
  ...
  const [confirmDelSep,  setConfirmDelSep]  = useState(null)
  ...
  const fetchSeparations = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('*, employees(full_name, employee_id), separation_templates(template_title)')
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('*, departments(name), designations(title), separation_template_activities(id)')
  ...
  useEffect(() => {
    ...
    fetchSeparations()
    fetchTemplates()
  }, [fetchEmployees, fetchDepts, fetchDesignations, fetchSeparations, fetchTemplates])
  ...
  const handleTemplateSelect = async (templateId) => {
    ...
    const { data } = await supabase
      .from('separation_template_activities')
      .select('*')
  ...
  const handleCreateSeparation = async () => {
    ...
    if (!newForm.resignation_date) { toast.error('Resignation date is required'); return }
    setSaving(true)
    try {
      const suffix = String(Date.now()).slice(-6)
      const sepId = crypto.randomUUID()
      await supabase.from('employee_separations').insert([{
        id: sepId,
        separation_number: `SEP-${suffix}`,
        employee_id: newForm.employee_id,
        template_id: newForm.template_id || null,
        resignation_date: newForm.resignation_date,
        last_working_day: newForm.last_working_day || null,
        reason: newForm.reason,
        status: 'Pending',
        created_by: user?.full_name || '',
      }])
      if (newActivities.length > 0) {
        await supabase.from('separation_activities').insert(
  ...
  const openDetail = async (sep) => {
    setDetail(sep)
    setDetailExitNotes(sep.exit_interview || '')
    setDetailSettlement(sep.final_settlement_date || '')
    setLoadingDetail(true)
    const { data } = await supabase
      .from('separation_activities')
      .select('*')
  ...
  const handleActivityStatus = async (actId, newStatus) => {
    ...
    await supabase.from('separation_activities').update({ status: newStatus, completion_date: completionDate }).eq('id', actId)
    ...
    if (allDone) {
      await supabase.from('employee_separations').update({ status: 'Completed' }).eq('id', detail.id)
      setDetail(d => ({ ...d, status: 'Completed' }))
      setConfirmTerminate(true)
    } else if (done > 0) {
      await supabase.from('employee_separations').update({ status: 'In Progress' }).eq('id', detail.id)
      setDetail(d => ({ ...d, status: 'In Progress' }))
    }
    fetchSeparations()
  }

  const handleSaveDetailInfo = async () => {
    await supabase.from('employee_separations').update({
      exit_interview: detailExitNotes,
      final_settlement_date: detailSettlement || null,
    }).eq('id', detail.id)
    toast.success('Saved')
    setDetail(d => ({ ...d, exit_interview: detailExitNotes, final_settlement_date: detailSettlement }))
    fetchSeparations()
  }

  const handleTerminateEmployee = async () => {
    if (!detail) return
    await supabase.from('employees').update({ status: 'Terminated' }).eq('id', detail.employee_id)
  ...
  const handleDeleteSeparation = async () => {
    await supabase.from('separation_activities').delete().eq('separation_id', confirmDelSep.id)
    await supabase.from('employee_separations').delete().eq('id', confirmDelSep.id)
    toast.success('Separation deleted')
    setConfirmDelSep(null)
    fetchSeparations()
  }
  ...
  const handleSaveTmpl = async () => {
    ...
    try {
      ...
      if (editTmpl) {
        await supabase.from('separation_templates').update(payload).eq('id', editTmpl.id)
        toast.success('Template updated')
      } else {
        const newId = crypto.randomUUID()
        await supabase.from('separation_templates').insert([{
  ...
  const handleSaveTmplAct = async () => {
    ...
    if (editTmplAct) {
      await supabase.from('separation_template_activities').update(payload).eq('id', editTmplAct.id)
    } else {
      await supabase.from('separation_template_activities').insert([{
    ...
    setTmplActForm(emptyActivityForm())
    const { data } = await supabase
      .from('separation_template_activities').select('*').eq('template_id', editTmpl.id).order('sort_order')
    setTmplActivities(data || [])
    fetchTemplates()
  }

  const handleDeleteTmplAct = async (actId) => {
    await supabase.from('separation_template_activities').delete().eq('id', actId)
    setTmplActivities(prev => prev.filter(a => a.id !== actId))
    fetchTemplates()
  }

  const handleDeleteTmpl = async () => {
    await supabase.from('separation_template_activities').delete().eq('template_id', confirmDelTmpl.id)
    await supabase.from('separation_templates').delete().eq('id', confirmDelTmpl.id)
  ...
  return (
    <div>
      <PageHeader title="Employee Separation">
        {canEdit && mainTab === 'separations' && (
          <button className="btn btn-primary btn-sm" onClick={() => { setNewForm(emptyNewForm()); setNewActivities([]); setShowNew(true) }}>
            <span className="material-icons">add</span> New Separation
          </button>
        )}
      ...
      {mainTab === 'separations' && (
        <div style={{ marginTop: 16 }}>
          ...
          {loadingSep ? (
            ...
            <EmptyState icon="exit_to_app" message="No separations yet." />
          ) : (
            <div className="table-wrap">
              <table className="stock-table">
                ...
                <tbody>
                  {separations.map(sep => (
                    <tr key={sep.id}>
                      <td style={{ fontWeight: 600, color: 'var(--gold)', cursor: 'pointer' }} onClick={() => openDetail(sep)}>{sep.separation_number}</td>
                      <td>{sep.employees?.full_name || '—'}</td>
                      <td>{sep.resignation_date || '—'}</td>
                      <td>{sep.last_working_day || '—'}</td>
                      <td style={{ fontSize: 12, color: 'var(--text-dim)' }}>{sep.separation_templates?.template_title || '—'}</td>
                      <td>
                        <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 4, background: `${statusColor(sep.status)}18`, color: statusColor(sep.status), border: `1px solid ${statusColor(sep.status)}44` }}>
                          {sep.status}
                        </span>
                      </td>
                      <td style={{ display: 'flex', gap: 4 }}>
                        <button className="btn btn-xs btn-secondary" onClick={() => openDetail(sep)}>
                          <span className="material-icons" style={{ fontSize: 13 }}>visibility</span>
                        </button>
                        {canEdit && (
                          <button className="btn btn-xs btn-danger" onClick={() => setConfirmDelSep(sep)}>
      ...
      {mainTab === 'templates' && (
        <div style={{ marginTop: 16 }}>
          {loadingTmpls ? (
            ...
            <EmptyState icon="description" message="No separation templates yet." />
          ) : (
            <div className="table-wrap">
              <table className="stock-table">
                <thead>
                ...
                <tbody>
                  {templates.map(t => (
                    <tr key={t.id} style={{ opacity: t.is_active ? 1 : 0.6 }}>
                      ...
                      <td>{t.separation_template_activities?.length || 0}</td>
                      <td>
      ...
      <ModalDialog open={showNew} onClose={() => setShowNew(false)} title="New Separation" size="lg">
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className="form-group">
              ...
              <input type="date" className="form-control" value={newForm.resignation_date} onChange={e => setNewForm(f => ({ ...f, resignation_date: e.target.value }))} />
            </div>
            <div className="form-group">
              <label>Last Working Day</label>
              <input type="date" className="form-control" value={newForm.last_working_day} onChange={e => setNewForm(f => ({ ...f, last_working_day: e.target.value }))} />
            </div>
          </div>
          <div className="form-group">
            <label>Reason</label>
            <textarea className="form-control" rows={2} value={newForm.reason} onChange={e => setNewForm(f => ({ ...f, reason: e.target.value }))} />
          </div>
          <div className="form-group">
        ...
        <ModalActions>
          ...
          <button className="btn btn-primary" onClick={handleCreateSeparation} disabled={saving}>{saving ? 'Creating…' : 'Create'}</button>
        </ModalActions>
      </ModalDialog>

      {detail && (
        <ModalDialog open={!!detail} onClose={() => setDetail(null)} title={`${detail.separation_number} · ${detail.employees?.full_name || ''}`} size="lg">
          <div style={{ padding: 20 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 16 }}>
              <div style={{ fontSize: 12 }}><span style={{ color: 'var(--text-dim)' }}>Resignation:</span> <strong>{detail.resignation_date || '—'}</strong></div>
              <div style={{ fontSize: 12 }}><span style={{ color: 'var(--text-dim)' }}>Last Day:</span> <strong>{detail.last_working_day || '—'}</strong></div>
            ...
            {detail.reason && (
              <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 16 }}>
                <span style={{ fontWeight: 600 }}>Reason:</span> {detail.reason}
              </div>
            )}

            <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 10 }}>Activities</div>
            {loadingDetail ? <Spinner /> : detailActivities.length === 0 ? (
              <EmptyState icon="checklist" message="No activities for this separation." />
            ) : (
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div className="form-group">
                <label style={{ fontSize: 12 }}>Exit Interview Notes</label>
                <textarea
                  className="form-control"
                  rows={3}
                  value={detailExitNotes}
                  onChange={e => setDetailExitNotes(e.target.value)}
                />
              </div>
              <div className="form-group">
                <label style={{ fontSize: 12 }}>Final Settlement Date</label>
                <input
                  type="date"
                  className="form-control"
                  value={detailSettlement}
                  onChange={e => setDetailSettlement(e.target.value)}
                />
              </div>
              {canEdit && (
                <button className="btn btn-primary btn-sm" style={{ alignSelf: 'flex-start' }} onClick={handleSaveDetailInfo}>
                  Save Notes &amp; Settlement
                </button>
              )}
            </div>
          </div>
      ...
      <ModalDialog open={showTmplModal} onClose={() => setShowTmplModal(false)} title={editTmpl ? `Edit: ${editTmpl.template_title}` : 'New Separation Template'} size="lg">
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="form-group">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input type="checkbox" id="sep_tmpl_active" checked={tmplForm.is_active} onChange={e => setTmplForm(f => ({ ...f, is_active: e.target.checked }))} />
            <label htmlFor="sep_tmpl_active" style={{ margin: 0 }}>Active</label>
          </div>
              ...
              {editTmplAct !== null && (
                <div style={{ padding: 12, background: 'var(--surface)', borderRadius: 8, border: '1px solid var(--border)', marginBottom: 12, display: 'grid', gridTemplateColumns: '1fr auto auto auto auto', gap: 8, alignItems: 'flex-end' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, paddingBottom: 2 }}>
                    <input type="checkbox" id="sep_act_req" checked={tmplActForm.required} onChange={e => setTmplActForm(f => ({ ...f, required: e.target.checked }))} />
                    <label htmlFor="sep_act_req" style={{ margin: 0, fontSize: 12 }}>Required</label>
                  </div>
      ...
      <ConfirmDialog
        ...
        title="Mark Employee as Terminated?"
        message={`All activities are complete. Would you like to mark ${detail?.employees?.full_name || 'this employee'} as Terminated?`}
        confirmLabel="Terminate"
        danger
      />
      <ConfirmDialog open={!!confirmDelSep} onClose={() => setConfirmDelSep(null)} onConfirm={handleDeleteSeparation}
        title="Delete Separation" message={`Delete separation ${confirmDelSep?.separation_number}?`} confirmLabel="Delete" danger />
