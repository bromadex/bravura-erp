...
export default function JournalEntries() {
  ...
  return (
    <div style={{ padding: 24 }}>
      <PageHeader title="Journal Entries" subtitle={`${journalEntries.length} entries`}>
        {canEdit && (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid var(--border)' }}>
                          ...
                          <th className="th-right" style={{ padding: '8px 18px', color: 'var(--text-dim)', fontWeight: 600 }}>Debit</th>
                          <th className="th-right" style={{ padding: '8px 18px', color: 'var(--text-dim)', fontWeight: 600 }}>Credit</th>
                        </tr>
                      </thead>
                      <tbody>
                        {eLines.map(l => (
                          <tr key={l.id} style={{ borderBottom: '1px solid var(--border)' }}>
                            ...
                            <td className="td-right" style={{ padding: '7px 18px', fontFamily: 'var(--mono)', color: l.debit > 0 ? 'var(--green)' : 'var(--text-dim)' }}>{l.debit > 0 ? fmt(l.debit) : '—'}</td>
                            <td className="td-right" style={{ padding: '7px 18px', fontFamily: 'var(--mono)', color: l.credit > 0 ? 'var(--red)' : 'var(--text-dim)' }}>{l.credit > 0 ? fmt(l.credit) : '—'}</td>
                          </tr>
                        ))}
                        <tr style={{ background: 'var(--surface2)', fontWeight: 700 }}>
                          <td colSpan={2} style={{ padding: '7px 18px', color: 'var(--text-dim)' }}>Total</td>
                          <td className="td-right" style={{ padding: '7px 18px', fontFamily: 'var(--mono)', color: 'var(--green)' }}>{fmt(e.total_debit)}</td>
                          <td className="td-right" style={{ padding: '7px 18px', fontFamily: 'var(--mono)', color: 'var(--red)' }}>{fmt(e.total_credit)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
      ...
      {/* New Entry modal */}
      <ModalDialog open={modal} onClose={() => setModal(false)} title="New Journal Entry" size="lg">
