...
export default function AssignRoomModal({ onClose, prefillEmployee = null, prefillRoom = null }) {
  ...
  useEffect(() => {
    supabase.from('employees').select('id, name, employee_number, gender, status').eq('status', 'active').order('name').then(({ data }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      const code = await assignRoom({ employeeId, roomId, startDate, notes, processedBy: user?.name })
