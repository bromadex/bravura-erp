...
export function HRProvider({ children }) {
  ...
  const runMonthlyAccrual = async (month, year, employeeIdFilter = null) => {
    // Accrue 2.5 days for each active employee for each active leave type.
    // The leave_accruals table has UNIQUE(employee_id, leave_type_id, month, year)
    // so duplicate runs are safe — the insert is ignored.

    const activeEmps = employees.filter(e =>
      e.status === 'Active' &&
      (!employeeIdFilter || e.id === employeeIdFilter)
    )
    if (!activeEmps.length) throw new Error('No active employees found')

    const { data: ltData } = await supabase.from('leave_types').select('id').eq('is_active', true)
    const leaveTypeIds = (ltData || []).map(lt => lt.id)
    if (!leaveTypeIds.length) throw new Error('No active leave types configured')
    ...
    for (const emp of activeEmps) {
      for (const ltId of leaveTypeIds) {
        // Insert accrual record — ON CONFLICT skips duplicates
        ...
        if (accrualError && !accrualError.message?.includes('duplicate key')) {
          console.warn('Accrual insert error:', accrualError.message)
          continue
        }
        if (accrualError) continue  // already accrued this month

        // Add 2.5 days to leave_balances total_days for the relevant year
        ...
        // If upsert created a new record that's fine. If it updated an existing
        // one we need to ADD 2.5, not set it. Use RPC or raw update:
        if (!balError) {
          await supabase.rpc('increment_leave_balance', {
            ...
            // If RPC doesn't exist yet, fallback: fetch then update
            supabase
              .from('leave_balances')
              .select('total_days')
              ...
              .then(({ data: bal }) => {
                if (bal) {
                  supabase
                    .from('leave_balances')
                    .update({ total_days: (bal.total_days || 0) + 2.5 })
                    .eq('employee_id', emp.id)
                    .eq('leave_type_id', ltId)
                    .eq('year', currentYear)
                }
    ...
    await logHRAction('MONTHLY_ACCRUAL', 'leave_accrual', `${month}/${year}`, `Accrual ${month}/${year}`, null, { month, year, records: accrued })
    await fetchAll()
    return accrued
  }
