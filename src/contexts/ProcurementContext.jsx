...
export function ProcurementProvider({ children }) {
  ...
  const approveStoreRequisition = async (id, approverName, approverId) => {
    ...
    // Check deficits and auto-create PRs
    const deficits = []
    ...
    // Auto-create Purchase Requisitions for deficits
  ...
  const rejectStoreRequisition = async (id, reason, approverName, approverId) => {
    const { error } = await supabase.from('store_requisitions').update({
      ...
      approver_name: approverName, rejection_reason: reason,
      updated_at: new Date().toISOString(),
    }).eq('id', id)
    if (error) throw error
    await fetchAll()
    ...
    for (const it of grn.items) {
      ...
      if (!existing) {
        await supabase.from('items').insert([{
          ...
          unit: it.unit || 'pcs', balance: it.received,
          total_in: it.received, total_out: 0,
      ...
      await supabase.from('transactions').insert([{
        id: generateId(), type: 'GRN', item_name: it.name,
        category: it.category, qty: it.received,
        date: grn.date, delivered_by: grn.supplier_name || grn.driver || '',
        received_by: grn.received_by || '', notes: `GRN: ${grnNumber}`,
        user_name: grn.received_by || '', created_at: new Date().toISOString(),
      }])
    }
  ...
  return (
    <ProcurementContext.Provider value={{
      suppliers, storeRequisitions, purchaseRequisitions,
      purchaseOrders, goodsReceived, loading,
      addSupplier, updateSupplier, deleteSupplier,
      createStoreRequisition, updateStoreRequisition,
      approveStoreRequisition, rejectStoreRequisition,
      approvePurchaseRequisition, rejectPurchaseRequisition,
      createPurchaseOrder, updatePurchaseOrderStatus,
      createGoodsReceived, fetchAll,
    }}>
      {children}
