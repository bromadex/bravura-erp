...
//
// TIER 1 FIX:
// - Added approveAttendance, rejectAttendance, bulkApproveAttendance.
//   These were referenced in Attendance.jsx but missing from this context,
//   causing "approveAttendance is not a function" runtime crashes.
// - All three functions take canApprove as the LAST argument and throw if
//   the user lacks permission, matching the call signatures in Attendance.jsx.
// - Each function only mutates rows where status = 'pending' (DB-level guard
//   so even concurrent clicks can't re-approve already-actioned records).
//
// Everything else is unchanged from your existing HRContext.
...
export function HRProvider({ children }) {
  ...
  const generateEmployeeNumber = async () => {
    ...
    if (error) return 'BRA164'
    let maxNum = 0
    data?.forEach(emp => {
      if (emp.employee_number) {
        const num = parseInt(emp.employee_number.replace('BRA', ''), 10)
  ...
  const addEmployee = async (employee, createAccount = false, accountRoleId = 'role_viewer', initialBalances = []) => {
    ...
    if (allLeaveTypes?.length) {
      ...
      for (const lt of allLeaveTypes) {
        ...
        balanceInserts.push({
          id: generateId(), employee_id: id, leave_type_id: lt.id,
          total_days: totalDays, used_days: 0, year: currentYear
        })
        balanceInserts.push({
          id: generateId(), employee_id: id, leave_type_id: lt.id,
          total_days: 0, used_days: 0, year: currentYear - 1
        })
      }
  ...
  // ─────────────────────────────────────────────────────────────────
  // ✅ TIER 1 FIX — Timesheet approval functions
  // Signatures match Attendance.jsx exactly:
  //   approveAttendance(recordId, approverName, canApprove)
  //   rejectAttendance(recordId, approverName, reason, canApprove)
  //   bulkApproveAttendance(recordIds, approverName, canApprove)
  // ─────────────────────────────────────────────────────────────────
  ...
  const approveAttendance = async (recordId, approverName, canApprove) => {
    ...
    const { error } = await supabase
      ...
      .eq('id', recordId)
      .eq('status', 'pending')   // DB-level guard: only pending records can be approved

    if (error) throw new Error(error.message)
    await fetchAll()
    await logHRAction(
      'APPROVE_ATTENDANCE', 'attendance', recordId, '', null,
      { status: 'approved', approved_by: approverName }
    )
  }
  ...
  const rejectAttendance = async (recordId, approverName, reason, canApprove) => {
    ...
    const { error } = await supabase
      ...
      .eq('id', recordId)
      .eq('status', 'pending')   // DB-level guard

    if (error) throw new Error(error.message)
    await fetchAll()
    await logHRAction(
      'REJECT_ATTENDANCE', 'attendance', recordId, '', null,
  ...
  const bulkApproveAttendance = async (recordIds, approverName, canApprove) => {
    ...
    await fetchAll()
    await logHRAction(
      'BULK_APPROVE_ATTENDANCE', 'attendance', recordIds.join(','), '', null,
      { count: recordIds.length, approved_by: approverName }
    )
  }
  ...
  const getPendingForSupervisor = async (supervisorEmployeeId) => {
    const { data: dept } = await supabase
      .from('departments')
      .select('id')
      .eq('hod_id', supervisorEmployeeId)
      .maybeSingle()
    if (!dept) return []
    ...
    if (!employeesInDept?.length) return []
  ...
  const approveLeaveRequest = async (requestId, approverEmployeeId, approverName, comment = null) => {
    ...
    if (newStatus === 'approved') {
      const hasClockedIn = attendance.some(a =>
        a.employee_id === request.employee_id && a.date === request.start_date && a.clock_in
      )
  ...
  return (
    <HRContext.Provider value={{
      ...
      // ✅ Tier 1 — newly added attendance approval methods
